...
## Команды управления возвратами

REST-эндпоинт `POST /api/v1/returns/{id}/commands` принимает команды в формате:
...
```json
{
  ...
  "payload": {
    "reverseTrack": "BY123456789",
    "comment": "Комментарий менеджера"
  }
}
```

- Поле `payload` теперь представляет собой произвольный JSON-объект. Для команд,
  не требующих параметров (`SET_MODE_EXCHANGE`, `CLOSE_REQUEST` и другие), объект можно
  опустить или передавать пустой.
- Команда `UPDATE_DETAILS` требует наличия хотя бы одного поля из
  `reverseTrack` или `comment`. Пустой объект будет отклонён.
- Значение `reverseTrack` нормализуется: пробелы убираются, строка приводится к
  верхнему регистру и проверяется на длину не более 64 символов.
- Значение `comment` очищается от лишних пробелов и ограничено 2000 символами.

При нарушении правил фабрика `ReturnRequestCommandPayloadFactory` выбрасывает
`IllegalArgumentException`. Контроллер превращает её в ответ 400 с понятным
сообщением об ошибке, что упрощает диагностику интеграций.
