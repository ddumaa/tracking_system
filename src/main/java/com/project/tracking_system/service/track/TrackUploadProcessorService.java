...
public class TrackUploadProcessorService {
     ...
     * <p>
     * Метод возвращает список некорректных строк, собранных во время валидации,
     * чтобы контроллер мог отобразить их пользователю или передать через WebSocket.
     * Если после фильтрации не осталось подходящих треков,
     * регистрируется пустая партия, отправляется сообщение об ошибке,
     * и дальнейшая обработка не выполняется.
     * </p>
