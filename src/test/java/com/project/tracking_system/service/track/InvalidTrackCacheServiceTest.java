...
class InvalidTrackCacheServiceTest {
    ...
    void removeExpired_RespectsUpdatedSetting() {
        ...
        // гарантируем, что метка времени последнего обращения изменится
        // иначе при нулевом TTL метод может не удалить запись, если время
        // обращения совпадёт с текущим временем в пределах миллисекунды
        // (System.currentTimeMillis может вернуть тот же результат)
        try {
            Thread.sleep(1);
